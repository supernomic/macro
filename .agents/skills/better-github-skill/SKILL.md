...
> **Local note (this repo):** if `node script.ts` fails with
> `ERR_UNKNOWN_FILE_EXTENSION` (Node < 23.6), run the scripts with
> `node --experimental-strip-types scripts/<name>.ts` instead.
